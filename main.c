...
/*---------------------------------------*/
static float32_t GetTempData(int C_or_F);//CELSIUS, FAHRENHEIT
static void TimerInit(uint32_t period);//timer set configuration data
    ...
    for(;;)
    {
    	if(btn_event == true)
    	{
    	    ...
    	    if(out_config_register == CELSIUS){
    	    	out_config_register = 1u;
    	    	printf("FAHRENHEIT\r\n");
    	    }else{
    	    	out_config_register = 0u;
    	    	printf("CELSIUS\r\n");
    	    }
    	}
    	...
    	if(tim_event == true)
    	{
    		tim_event = false;
    			...
    			temp_to_out = GetTempData(out_config_register);
    			printf("<%02d-%02d-20%02d %02d:%02d:%02d> <%0.1f>\r\n", (int)timeStr.date,(int)timeStr.month,(int)timeStr.year,(int)timeStr.hour,(int)timeStr.min,(int)timeStr.sec,temp_to_out);
...
static float32_t GetTempData(int c_or_f)
{
	uint16_t adc_data = 0;
	uint16_t adcstat = 0;
	float32_t tmp_to_ret = 0;
	...
	if(c_or_f == 0u){
		tmp_to_ret = (float)(adc_data)/14.8-43.65;
	}else{
		tmp_to_ret = (float)(adc_data)/14.8-43.65;
		tmp_to_ret = tmp_to_ret * 1.8 + 32.0;
	}
	return tmp_to_ret;
}

static void TimerInit(uint32_t period)
{//32 bit timer the HW of kings)))
	cy_rslt_t result;
	 const cyhal_timer_cfg_t led_blink_timer_cfg = \
	    {
	        .compare_value = LED_INDICATION_TIME,                 /* Timer compare value, not used */
...
{
	cyhal_gpio_init((cyhal_gpio_t)BTN_HAL_PORT_PIN, BTN_HAL_DIR, BTN_HAL_DRIVEMODE, CYBSP_BTN_OFF);//Init BTN
	cyhal_gpio_register_callback((cyhal_gpio_t)BTN_HAL_PORT_PIN, gpio_interrupt_handler, NULL);//BTN Interrupt
	cyhal_gpio_enable_event((cyhal_gpio_t) BTN_HAL_PORT_PIN, CYHAL_GPIO_IRQ_FALL, INTERRUPT_PRIORITY, true);//I do not know about INTERRUPT_PRIORITY!!!
}
